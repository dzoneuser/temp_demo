...
public class ProductAController {
    ...
    public String test(){
        ...
        return "test__"+
                this.getClass().getName()+"_"+ LocalDateTime.now().toString()
                +"__"+response;

    }

    @PostMapping("v1/billing/{arg}")
    public ResponseEntity<BillingRequest> saveBilling
            (@RequestBody BillingRequest billingRequest, @PathVariable String arg)
            throws ExecutionException, InterruptedException {

        BillingRequest resp =service.saveBilling(billingRequest,arg);
    ...
    public ResponseEntity<BillingRequest>
    saveBillingSSO(@RequestBody BillingRequest billingRequest, @PathVariable String arg)
            throws ExecutionException, InterruptedException {
        return new ResponseEntity<>(service.saveBilling(billingRequest,arg), HttpStatus.CREATED);
    }
